...
class ProductByBannerScreen extends StatefulWidget {
  final int fromBanner;
  const ProductByBannerScreen({Key? key, required this.fromBanner})
      : super(key: key);

  @override
  _ProductByBannerScreenState createState() => _ProductByBannerScreenState();
}

class _ProductByBannerScreenState extends State<ProductByBannerScreen> {
  ...
  void initState() {
    ...
    switch (widget.fromBanner) {
      case 0:
        {
          _title = "Impressive consoles";
          _actualItems = _productController.productList
              .where((p0) =>
                  p0.categoryId == 22 ||
                  p0.categoryId == 36 ||
                  p0.categoryId == 37)
    ...
    return Scaffold(
      ...
      body: GridView.builder(
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          ...
          mainAxisExtent: 238 * screenScale(context),
        ),
        shrinkWrap: true,
  ...
  AppBar _customizedAppBar() {
    return AppBar(
      actions: [
        GestureDetector(
          onTap: () => showSearch(
              context: context, delegate: AppSearchBar(items: _actualItems)),
          child: Padding(
  ...
  Widget _productCard({required Product product}) {
    return Container(
      height: 235 * screenScale(context),
